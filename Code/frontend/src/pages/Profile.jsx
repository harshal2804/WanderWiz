          ...
          <div style={functionbutton}>
            <ButtonToolbar aria-label="Toolbar with button groups">
              ...
              <ButtonGroup className="me-2" aria-label="Third group">
                <Button onClick={(e) => handleLogOut(e)} variant="danger" type="reset">
                  Log Out
                </Button>
              </ButtonGroup>
            </ButtonToolbar>
            ...
            {data.itineraries.map((element) => {
              return (
                <div onClick={(e) => handleIti(e, element._id)} key={element._id}>
                  <Itinerary
                    title={element.name} description={Math.floor((Date.parse(element.endDate) - Date.parse(element.startDate))/86400000) + 1} 
                    imageUrl={element.photo} newsUrl={element.url}
                  />
                </div>
              )
            })}
